...
window.onload = function () {
	...
	for (let n1 = 0; n1 < gSettings.sections.length; n1++) {
		...
		for (let n2 = 0; n2 < gSettings.sections[n1].tiles.length; n2++) {
			...
			// Extract protocol and port for constructing the URL
			let protocol = gSettings.sections[n1].tiles[n2].protocol;
			let port = gSettings.sections[n1].tiles[n2].port;
			let path = gSettings.sections[n1].tiles[n2].path;
			let hostname = window.location.hostname; // Get the current hostname

			// Construct the URL dynamically
			let thisUrl = `${protocol}://${hostname}:${port}${path}`;
...
function createSections() {
	...
	for (let n1 = 0; n1 < gSettings.sections.length; n1++) {
		...
		for (let n2 = 0; n2 < gSettings.sections[n1].tiles.length; n2++) {
			...
			// Construct the URL using the protocol and port
			let url = `${protocol}://${window.location.hostname}:${port}${path}`;
...
function refreshWidgetGlances(nW) {
	let protocol = gSettings.widgets[nW].protocol;
    let port = gSettings.widgets[nW].port;
    let path = gSettings.widgets[nW].path;

    // Construct the widget's URL dynamically
    let url = `${protocol}://${window.location.hostname}:${port}${path}`;

	$.getJSON({ url: url + 'api/4/quicklook' }).done(function (result, status, xhr) {
		document.getElementById('cpuPrct' + nW).innerText = result.cpu + '%'
		document.getElementById('memPrct' + nW).innerText = result.mem + '%'
	});

	$.getJSON({ url: url + 'api/4/sensors' }).done(function (result, status, xhr) {
